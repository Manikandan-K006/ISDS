    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="gradient-hero rounded-2xl p-6 lg:p-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold theme-text font-heading">Manage Assignments</h1>
            <p className="theme-text mt-1">Create and grade assignments</p>
          </div>
      ...
      <div className="space-y-3">
        {filtered.map(a => (
          <div key={a._id} className="glass rounded-xl p-4 border theme-border">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-semibold theme-text">{a.title}</h3>
                  <p className="text-xs theme-text-muted">{a.courseName || courses.find(c => c._id === a.courseId)?.title || ''}</p>
                </div>
              </div>
              <div className="flex gap-1">
                ...
                <button onClick={() => handleDelete(a._id)}
                  className="p-1.5 rounded-lg hover:bg-rose-500/10 theme-text-muted hover:text-rose-400">
                  <FiTrash2 size={14} />
                </button>
              </div>
            </div>
            <p className="text-xs theme-text-muted mt-2 line-clamp-1">{a.description}</p>
      ...
      <Modal isOpen={showForm} onClose={() => setShowForm(false)} title={editing ? 'Edit Assignment' : 'Create Assignment'}>
        <div className="space-y-4">
          <div>
            <label className="block text-xs theme-text-muted mb-1">Course</label>
            <select value={form.courseId} onChange={e => setForm({...form, courseId: e.target.value})}
              className="w-full theme-input border theme-border rounded px-3 py-2 text-sm theme-text focus:outline-none focus:border-indigo-500/50">
              {courses.map(c => <option key={c._id} value={c._id}>{c.title}</option>)}
            </select>
          </div>
          {['title', 'description'].map(f => (
            <div key={f}>
              <label className="block text-xs theme-text-muted mb-1 capitalize">{f}</label>
              {f === 'description' ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs theme-text-muted mb-1">Deadline</label>
              <input type="datetime-local" value={form.deadline} onChange={e => setForm({...form, deadline: e.target.value})}
                className="w-full theme-input border theme-border rounded px-3 py-2 text-sm theme-text focus:outline-none focus:border-indigo-500/50" />
            </div>
            <div>
              <label className="block text-xs theme-text-muted mb-1">Max Marks</label>
              <input type="number" value={form.maxMarks} onChange={e => setForm({...form, maxMarks: +e.target.value})}
                className="w-full theme-input border theme-border rounded px-3 py-2 text-sm theme-text focus:outline-none focus:border-indigo-500/50" />
            </div>
          </div>
          <div>
            <label className="block text-xs theme-text-muted mb-1">Submission Type</label>
