...
const ParentList = () => {
  ...
  useEffect(() => {
    setLoading(true);
    API.get('/admin/users', { params: { role: 'parent', page, limit: 20, search: search || undefined } })
      .then(res => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data?.user) {
        setParents(prev => prev.map(p => p.id === data.user.id ? { ...p, ...data.user } : p));
      }
      setEditOpen(false);
      setEditing(null);
    } catch (err) {
  ...
  const handleDeactivate = (p) => {
    ...
    API.delete(`/admin/users/${p.id}`)
      .then(() => setParents(prev => prev.filter(x => x.id !== p.id)))
      .catch(console.error);
  };
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="gradient-hero rounded-2xl p-6 lg:p-8 relative overflow-hidden">
        ...
        <div className="relative">
          <h1 className="text-2xl lg:text-3xl font-bold theme-text font-heading">Parents</h1>
          <p className="theme-text mt-1">Manage parent and guardian accounts</p>
        </div>
      </motion.div>
      ...
      {parents.length === 0 ? (
        <div className="theme-card border theme-border rounded-2xl p-12 text-center">
          ...
          <p className="theme-text-muted text-lg">No parents found</p>
          <p className="theme-text-muted text-sm mt-1">Parents will appear here once they register.</p>
        </div>
      ) : (
        <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b theme-border">
                    <th className="p-3 pl-5 text-left text-xs theme-text-muted font-medium uppercase tracking-wider">Parent</th>
                    ...
                    >
                      <td className="p-3 pl-5">
                        <div className="flex items-center gap-3">
                          {p.profilePhoto ? (
                            <img src={p.profilePhoto} alt={p.name} className="w-9 h-9 rounded-full object-cover shrink-0" />
                          ) : (
                            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500/20 to-purple-500/20 flex items-center justify-center shrink-0">
                              <span className="text-xs font-bold text-indigo-400">{p.name?.split(' ').map(n => n[0]).join('')}</span>
                            </div>
                          )}
                          <div>
                            <p className="text-sm theme-text font-medium">{p.name}</p>
                            <p className="text-xs theme-text-muted">{p.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="p-3">
                        <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${getStatusStyle(p.isActive)}`}>
                          {p.isActive ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="p-3">
                        <span className="text-xs theme-text-muted">{p.createdAt ? new Date(p.createdAt).toLocaleDateString() : 'N/A'}</span>
                      </td>
                      <td className="p-3">
            ...
            {parents.map((p, i) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    {p.profilePhoto ? (
                      <img src={p.profilePhoto} alt={p.name} className="w-10 h-10 rounded-full object-cover shrink-0" />
                    ) : (
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500/20 to-purple-500/20 flex items-center justify-center shrink-0">
                        <span className="text-sm font-bold text-indigo-400">{p.name?.split(' ').map(n => n[0]).join('')}</span>
                      </div>
                    )}
                    <div>
                      <p className="text-sm theme-text font-medium">{p.name}</p>
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${getStatusStyle(p.isActive)}`}>
                    {p.isActive ? 'Active' : 'Inactive'}
                  </span>
                </div>
                <div className="theme-subtle rounded-xl p-2 text-center mb-3">
                  <p className="text-[10px] theme-text-muted mb-0.5">Joined</p>
                  <p className="text-sm theme-text font-medium">{p.createdAt ? new Date(p.createdAt).toLocaleDateString() : 'N/A'}</p>
                </div>
          ...
          <div className="flex items-center justify-between text-xs theme-text-muted">
            <span>Showing {parents.length} of {total} parents</span>
      ...
      <Modal open={editOpen} onClose={() => { setEditOpen(false); setEditing(null); }} size="md">
        <div className="p-6">
          <h2 className="text-lg font-semibold theme-text mb-4">Edit Parent</h2>
