    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search courses..."
          className="w-full theme-card border theme-border rounded-xl pl-10 pr-3 py-2.5 text-sm theme-text placeholder-theme-muted focus:outline-none focus:border-indigo-500/40 transition-colors"
        />
      </div>
      ...
      <Modal isOpen={showForm} onClose={() => setShowForm(false)} title={editingCourse ? 'Edit Course' : 'Add Course'} size="xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input value={form.title} onChange={e => setForm({...form, title: e.target.value})}
                className="w-full theme-subtle theme-border rounded-xl px-3 py-2 text-sm theme-text placeholder-theme-muted focus:outline-none focus:theme-border-light transition-colors"
              />
            </div>
            <div>
              <label className="block text-xs theme-text-muted mb-1.5">Domain</label>
              <select value={form.domain} onChange={e => setForm({...form, domain: e.target.value})}
                className="w-full theme-subtle theme-border rounded-xl px-3 py-2 text-sm theme-text focus:outline-none focus:theme-border-light transition-colors">
                ...
                <div key={mi} className="theme-subtle rounded-xl p-3 border theme-border">
                  <div className="flex items-center gap-2">
                    <input value={mod.title} onChange={e => updateModule(mi, e.target.value)}
                      ...
                      className="flex-1 theme-subtle theme-border rounded-lg px-2 py-1.5 text-xs theme-text placeholder-theme-muted focus:outline-none focus:theme-border-light transition-colors"
                    />
                      ...
                      {mod.lessons.map((les, li) => (
                        <div key={li} className="flex items-center gap-1.5">
                          <input value={les.title} onChange={e => updateLesson(mi, li, 'title', e.target.value)}
                            ...
                            className="flex-1 theme-subtle theme-border rounded-lg px-2 py-1 text-xs theme-text placeholder-theme-muted focus:outline-none focus:theme-border-light transition-colors"
                          />
                          <input value={les.videoUrl} onChange={e => updateLesson(mi, li, 'videoUrl', e.target.value)}
                            placeholder="YouTube URL"
                            className="w-40 theme-subtle theme-border rounded-lg px-2 py-1 text-xs theme-text placeholder-theme-muted focus:outline-none focus:theme-border-light transition-colors"
                          />
        ...
        {activeSection === 'resources' && (
          <div>
            ...
            {form.resources.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center border border-dashed theme-border rounded-xl">
        ...
        <button onClick={handleSave} disabled={saveLoading}
          ...
          {saveLoading ? (
            <><div className="w-4 h-4 border-2 theme-border border-t-transparent rounded-full animate-spin" /> Saving...</>
          ) : (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea value={resourceForm.description} onChange={e => setResourceForm(prev => ({...prev, description: e.target.value}))}
              ...
              rows={2}
              className="w-full theme-subtle theme-border rounded-xl px-3 py-2 text-sm theme-text placeholder-theme-muted focus:outline-none focus:theme-border-light transition-colors resize-none" />
          </div>
          ...
          <div>
            ...
            <div
              ...
              className={`relative border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors ${
                dragOver ? 'border-indigo-400 bg-indigo-500/5' : 'theme-border hover:border-[var(--border-light)]'
              }`}
            >
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input value={resourceForm.externalUrl} onChange={e => setResourceForm(prev => ({...prev, externalUrl: e.target.value}))}
                ...
                className="w-full theme-subtle theme-border rounded-xl pl-9 pr-3 py-2 text-sm theme-text placeholder-theme-muted focus:outline-none focus:theme-border-light transition-colors" />
            </div>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={() => setResourceFormOpen(false)}
              className="flex-1 py-2 rounded-xl theme-border text-sm theme-text-muted hover:theme-text hover:theme-subtle transition-colors">
              Cancel
            </button>
